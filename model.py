...
class OGNet(nn.Module):
    ...
    def __init__(self, opt, dataloader):
        ...
        self.g = g_net().cuda() if torch.cuda.is_available() else g_net()
        self.d = d_net().cuda() if torch.cuda.is_available() else g_net()
        self.opt = opt

    def test_patches(self, g_model_path, d_model_path, i):
        # test all patches present inside a folder on given g and d model. Returns d score of each patch
        ...
        if torch.cuda.is_available():
            ...
            g_checkpoint = torch.load(g_model_path, map_location='cpu')
            self.g.load_state_dict(g_checkpoint['g_model_state_dict'])
        if torch.cuda.is_available():
            d_checkpoint = torch.load(d_model_path)
            self.d.load_state_dict(d_checkpoint['d_model_state_dict'])
        else:
            d_checkpoint = torch.load(d_model_path, map_location='cpu')
            self.d.load_state_dict(d_checkpoint['d_model_state_dict'])
        self.g.eval()
        self.d.eval()
        labels = []
